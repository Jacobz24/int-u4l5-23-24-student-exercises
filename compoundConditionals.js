...
let secretUsername = ("KanyeWestLover911");
let secretPassword = ("Drakesukslol");
...
button.addEventListener("click", function() {
    ...
    if (secretUsername === username && secretPassword === password) {
      document.querySelector(".message").innerHTML="Logged In!"
    }
  else {
    ...
    if (secretUsername === username && secretPassword === password) {
      ...
      else if (secretUsername === username && secretPassword !== password) {
        document.querySelector(".message").innerHTML="Wrong password - be careful, don't mess this up too many times."
      }
        else if (secretUsername !== username && secretPassword === password) {
          document.querySelector(".message").innerHTML="Wrong username - is there a typo?"
        }
          else if (username === "" && password === "") {
            document.querySelector(".message").innerHTML="You need to enter SOMETHING."
          }
    else {
      document.querySelector(".message").innerHTML="Incorrect username and password. Hmm. Suspicious."
    }
